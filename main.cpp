...
public:
	...
	Rational(const Rational & z); // об¤зательно ссылка (&)тк иначе будет посто¤нно вызыватьс¤ этот  метод из за необходимости копировани¤ копировани¤, const чтобы объект не изменилс¤
	int getNum() const { return num; }
	int getDenom() const { return denom; }


	Rational operator = (const Rational & orig) {// объ¤вление оператора снаружи класса
		//std::cout << num << " " << denom;
		num = orig.num;
		denom = orig.denom;
		Sokrach();
	...
	Rational operator++() {
		num = num + denom;
		return Rational(num, denom);
	} //(++c) дл¤ какой то из форм инкремента по¤вл¤етс¤ фейкова¤ переменна¤
